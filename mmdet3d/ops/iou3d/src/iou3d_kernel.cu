...
__global__ void paired_boxes_overlap_kernel(const int num, const float *boxes_a, const float *boxes_b, float *ans_overlap) {
...
void pairedboxesoverlapLauncher(const int num, const float *boxes_a, const float *boxes_b, float *ans_overlap) {
  dim3 blocks(DIVUP(num, THREADS_PER_BLOCK));  // blockIdx.x(col), blockIdx.y(row)
  dim3 threads(THREADS_PER_BLOCK);

  paired_boxes_overlap_kernel<<<blocks, threads>>>(num, boxes_a, boxes_b, ans_overlap);
#ifdef DEBUG
  cudaDeviceSynchronize();  // for using printf in kernel function
#endif
}
