...
class BaseInstance3DBoxes(object):
    ...
    @classmethod
    def height_overlaps(cls, boxes1, boxes2, mode='iou', paired=False):
        ...
        boxes1_top_height = boxes1.top_height.view(-1, 1) if not paired else boxes1.top_height.view(-1)
        boxes1_bottom_height = boxes1.bottom_height.view(-1, 1) if not paired else boxes1.bottom_height.view(-1)
        boxes2_top_height = boxes2.top_height.view(1, -1) if not paired else boxes2.top_height.view(-1)
        boxes2_bottom_height = boxes2.bottom_height.view(1, -1) if not paired else boxes2.bottom_height.view(-1)
    ...
    @classmethod
    def overlaps(cls, boxes1, boxes2, mode='iou', paired=False):
        ...
        if paired:
            assert rows == cols , 'The number of boxes1 and boxes2 should be ' \
                f'the same, got {rows} and {cols}.'

        if rows * cols == 0:
            return boxes1.tensor.new(rows, cols) if not paired else boxes1.tensor.new(rows) 

        # height overlap
        overlaps_h = cls.height_overlaps(boxes1, boxes2, paired=paired)
        ...
        if not paired:
            ...
            # bev overlap
            overlaps_bev = boxes1_bev.new_zeros((boxes1_bev.shape[0],)).cuda()  # (N,)
            iou3d_cuda.paired_boxes_overlap_bev_gpu(boxes1_bev.contiguous().cuda(), boxes2_bev.contiguous().cuda(), overlaps_bev)

        # 3d overlaps
        overlaps_3d = overlaps_bev.to(boxes1.device) * overlaps_h

        volume1 = boxes1.volume.view(-1, 1) if not paired else boxes1.volume.view(-1)
        volume2 = boxes2.volume.view(1, -1) if not paired else boxes2.volume.view(-1)
